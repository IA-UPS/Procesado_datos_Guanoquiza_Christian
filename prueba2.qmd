...
```{r}
fit <- cv.glmnet(X, y = F, alpha = 1, nfolds = 10)
```

```{r}
lambda_opt <- fit$lambda.min
fit2 <- glmnet(X, y, alpha = 1, lambda = lambda_opt)
y_pred <- predict(fit2, newx = X, type = "response")
accuracy <- mean(y == (y_pred > 0.5))
...
index <- createDataPartition(clase, p = 0.7, list = FALSE)
...
```{r}
X_train <- model.matrix(clase ~ ., data = datos.train)[,-1]
X_test <- model.matrix(clase ~ ., data = datos.test)[,-1]
y_train <- datos.train$V10
y_test <- datos.test$V10
