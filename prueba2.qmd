...
set.seed(123456789)
Lridge <- function(x) train(clase ~ ., data = x, method = "glmnet", trControl = trControl,tuneGrid = expand.grid(alpha=0,lambda=lambda_seq), trace = F)

logistica.lista <- lapply(entrenamiento,Lridge)
logisita.pred <- vector("list",length = length(datos.lista))
for(l in 1:length( datos.lista)){
  logisita.pred[[l]] <- predict(logistica.lista[[l]],test[[l]])
}
...
```{r}
matriz3d <- matrix(c(accuracy, ridgeaccuracy, lassoaccuracy), nrow = 3, ncol = 5, byrow = TRUE)
